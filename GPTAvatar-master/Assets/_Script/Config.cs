...
public class Friend : ScriptableObject
{
    ...
    public string _elevenLabsVoice = "";
    ...
    public void LoadConfigFile(string fileContents)
    {
        ...
        using (var reader = new StringReader(fileContents))
        {
            for (string line = reader.ReadLine(); line != null; line = reader.ReadLine())
            {
                ...
                {
                    //convert to float with TryParse using InvariantCulture
                    float.TryParse(words[1], NumberStyles.Float, CultureInfo.InvariantCulture, out friend._pitch);
                }
                else
                if (words[0] == "set_friend_voice_speed")
                {
                    //convert to float with TryParse using InvariantCulture
                    float.TryParse(words[1], NumberStyles.Float, CultureInfo.InvariantCulture, out friend._speed);
                }
                else
                ...
                {
                    //convert to int with TryParse
                    friend._elevenLabsVoice = words[1];
                }
                else
                ...
                {
                    //convert to float with TryParse using InvariantCulture
                    float.TryParse(words[1], NumberStyles.Float, CultureInfo.InvariantCulture, out friend._temperature);
                }
                else
                if (words[0] == "set_friend_elevenlabs_stability")
                {
                    //convert to float with TryParse using InvariantCulture
                    float.TryParse(words[1], NumberStyles.Float, CultureInfo.InvariantCulture, out friend._elevenlabsStability);
                }
